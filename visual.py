...
def handler_bem():
    forget_window_3()
    vobla.configure(state="normal")
    vobla.delete("1.0", "end")
    vobla.configure(state="disabled")
    show_window_2()
def handler_knopka():
    ...
    if widget_combobox.get() == "Азбука Морзе":
        vobla.configure(state="normal")
        vobla.insert("1.0", logic.morz(ar))
        vobla.configure(state="disabled")
    elif widget_combobox.get() == "Шифр Цезаря (шаг=3)":
        vobla.configure(state="normal")
        vobla.insert("1.0", logic.cesar(ar))
        vobla.configure(state="disabled")
    else:
        vobla.configure(state="normal")
        vobla.insert("1.0", logic.atbash(ar))
        vobla.configure(state="disabled")
    forget_window_2()
    show_window_3()
#---------------------------------------------------------------
...
opisanie.configure(text="Проект Мухаметзянова Эмиля, вводить текст только на русском языке!!!", font=iphone2)

widget_button = ctk.CTkButton(master=okno)
widget_button.configure(text="Начать", font=my_font, command=handler_widget_button)
###########################################################################################################
# виджеты для окна 2
elems = ["Азбука Морзе", "Шифр Цезаря (шаг=3)", "Шифр Атбаша"]  # список элементов
